...
- `80 m`: around 3.5–4.0 MHz, very popular at night for regional contacts
- `40 m`: around 7.0–7.3 MHz, good day and night for medium/long distance
- `20 m`: 14.0–14.35 MHz, classic worldwide DX band, very busy in daytime
- `15 m`: 21.0–21.45 MHz, good for DX when solar conditions are good
- `10 m:` 28.0–29.7 MHz, local plus long‑distance when propagation opens

Popular VHF/UHF bands (local)

- `2 m`: 144–148 MHz, main local FM/repeater band, plus weak‑signal SSB/CW.
- `70 cm`: 430–440 MHz or 420–450 MHz (regional differences), used for FM, repeaters, and satellites.
...
- `146.520 MHz`: 2 m FM national simplex calling frequency.
- `144.200 MHz`: 2 m SSB weak‑signal calling frequency.
- `446.000 MHz`: often used as a 70 cm FM calling/simplex frequency.
- around `14.300 MHz`: widely used for maritime and emergency nets on 20 m.

## Quick tutorial

- run `gqrx` software defined radio
- set center frequency to one from HAM radio bands
- select `AM`, `NFM`, `USB` or `USB` decoding modes
- listen and enjoy
